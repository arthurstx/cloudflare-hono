...
export class D1UsersRepository {
	...
	async findByEmail(email: string): Promise<User | null> {
		const query = `SELECT id, email, password_hash, role, created_at FROM users WHERE email = ?`;
		const result = await this.db.prepare(query).bind(email).first<User>();
		return result;
	}

	async findById(userId: string): Promise<User | null> {
		const query = `SELECT id, email, password_hash, role, created_at FROM users WHERE id = ?`;
